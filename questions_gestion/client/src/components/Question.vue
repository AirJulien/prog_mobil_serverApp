<template>
  <div class="Site">
    <Toolbar></Toolbar>
    <v-dialog v-if="isAuth" v-model="dialog" max-width="500px"
              @keydown.enter="save(nameNewQuestion,nameResponse1,nameResponse2,nameResponse3,radios)">
      ...
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="blue darken-1" flat
                 @click="save(nameNewQuestion,nameResponse1,nameResponse2,nameResponse3,radios)">Valider
          </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>

    <v-data-table v-if="isAuth" :headers="headers" :items="questions" hide-actions class="Site-content">
      <template slot="items" slot-scope="props">
        <tr>
          ...
          <td class="text-xs-left">{{ props.item.value_response1 === true ? "Réponse 1" : props.item.value_response2 ===
            true ? "Réponse 2" :"Réponse 3" }}
          </td>
          <td class="text-xs-left">
            <v-btn color="error" title="Supprimer?" @click="deleteQuestion(props.item._id)">X</v-btn>
          </td>
        </tr>
      </template>
    </v-data-table>
...
export default {
  computed: {
    isAuth: function () {
      if(this.$store.getters.isAuth) this.getAllQuestions()
  ...
  components: {Toolbar},
  created: function () {
    if (this.isAuth) this.getAllQuestions()
  },
  methods: {
    deleteQuestion (id) {
    ...
    getAllQuestions () {
      QuestionsService.fetchQuestions()
        .then(response => {
    ...
    save (nameNewQuestion, nameResponse1, nameResponse2, nameResponse3, radios) {
      ...
      let params = {
        ...
        value_response1: radios === '1',
        value_response2: radios === '2',
        value_response3: radios === '3'
      }
      QuestionsService.createQuestion(params)
        .then(response => {
          console.log(response)
          this.getAllQuestions()
        })
